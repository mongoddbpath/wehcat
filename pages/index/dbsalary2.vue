	...
	<view class="content">
		<view style="display: flex;width: 750rpx;justify-content: center;flex-direction: column;">
			...
			<view style="width: 750rpx;background: white;padding: 27rpx;display: flex;flex-direction: column;">
				...
				<view style="display: flex;width: 696rpx;align-items: center;" class="ok">
					<u-field v-model="allmoney" label="本期收入" :fieldStyle="{width:'490rpx'}" input-align="right" required
						:clearable="false" :error-message="errormessage1">
					</u-field>
				</view>
...
<script>
	...
	export default {
		...
		onLoad(o) {
			...
			var that = this
			this.id = o.id
			this.$u.post('Front/front/getsalaryinfo', {
				id: id
			}).then(res => {
				console.dir(res, {
					depth: null
				...
				this.item_no2 = getdata.item_no,
					...
					this.allmoney = getdata.allmoney,
					this.getmoney = getdata.getmoney,
					this.eip = getdata.eip,
					this.mip = getdata.mip,
					this.pmoney = getdata.pmoney,
					this.zfmoney = getdata.zfmoney,
					this.jzmoney = getdata.jzmoney,
					this.jzmoney2 = getdata.jzmoney2,
					this.interest = getdata.interest,
					this.homemoney = getdata.homemoney,
					this.oldmoney = getdata.oldmoney,
					this.bmoney = getdata.bmoney,
					this.hmoney = getdata.hmoney,
					this.symoney = getdata.symoney,
					this.othermonry = getdata.othermonry,
					this.jzemoney = getdata.jzemoney,
					this.jmsmoney = getdata.jmsmoney,
		...
		methods: {
			...
			submit() {
				...
				if (commonjs.if_obj_hasnot_arr(infodata2)) {
				...
				} else if (that.item_no2 != infodata2.item_no) {
					// 检查是否存在重复
					this.$u.post('Front/front/updatesalary2', infodata2).then(res => {
						if (parseInt(res.status) == 3) {
							...
							this.$u.post('Front/front/updatesalary', infodata2).then(res => {
								if (res.status) {
									uni.showToast({
										title: '编辑成功',
										icon: 'success',
